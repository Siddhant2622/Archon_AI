...
import { checkRateLimit, apiError, apiSuccess, getClientIP } from '@/lib/api/middleware';

export async function POST(req: NextRequest) {
  try {
    const clientIP = getClientIP(req);
    const rateLimit = checkRateLimit(`docs:${clientIP}`, { maxRequests: 5, windowMs: 60_000 });
    
    if (!rateLimit.allowed) {
      return apiError('Rate limit exceeded. Please wait before generating more documentation.', 429);
    }
