...
function getPasswordStrength(password: string): { label: string; score: number; color: string } {
  ...
  if (score <= 1) return { label: "Weak", score: 1, color: "#f43f5e" };
  if (score <= 2) return { label: "Fair", score: 2, color: "#fbbf24" };
  if (score <= 3) return { label: "Good", score: 3, color: "#22d3ee" };
  return { label: "Strong", score: 4, color: "#34d399" };
}
  ...
  const handleOAuth = async (provider: "google" | "github") => {
    ...
    try {
      ...
      if (err.code === "auth/account-exists-with-different-credential" || (err.message && err.message.includes("auth/account-exists-with-different-credential"))) {
        setError("An account already exists with the same email address. Please sign in using the provider you originally used.");
      } else {
  ...
  return (
    <div className="min-h-screen flex bg-bg-primary">
      ...
      >
        ...
        <div className="absolute top-1/2 right-1/3 w-64 h-64 bg-accent-violet/[0.06] rounded-full blur-[80px]" />
        <div className="absolute bottom-20 left-1/4 w-48 h-48 bg-accent-emerald/[0.06] rounded-full blur-[60px]" />
        ...
        <div className="relative z-10 space-y-8">
          ...
          <div className="space-y-5">
            {[
              "Deep code analysis in seconds",
              "Automated API documentation",
              "Real-time architecture visualization",
            ].map((f, i) => (
              <div key={i} className="flex items-center gap-3 text-text-secondary">
                <div className="w-6 h-6 rounded-full bg-accent-emerald/20 flex items-center justify-center flex-shrink-0">
        ...
        <div className="relative z-10">
          <p className="text-text-muted/50 text-sm">&copy; {new Date().getFullYear()} Archon AI. All rights reserved.</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12">
        <div className={`w-full max-w-md transition-all duration-700 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
          ...
          <p className="text-text-muted mb-6">Start your free trial. No credit card required.</p>

          {isDemo && (
            <div className="mb-6 p-4 bg-accent-cyan/10 border border-accent-cyan/20 rounded-xl text-sm text-accent-cyan">
              <strong>Demo Mode:</strong> Accounts will be saved locally on your device for testing purposes.
            </div>
          )}
          ...
          <form className="space-y-4" onSubmit={handleRegister}>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input type="text" className="base-input !bg-white/[0.02]" placeholder="acme-inc" value={formData.orgSlug} readOnly />
              </div>
            </div>
            <div>
              ...
              {formData.password && (
                <div className="mt-2">
                  <div className="flex gap-1 mb-1">
                    {[1, 2, 3, 4].map((i) => (
                      <div key={i} className="h-1.5 flex-1 rounded-full transition-all duration-300" style={{ background: i <= strength.score ? strength.color : "rgba(255,255,255,0.06)" }} />
                    ))}
                  </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className={`base-input !pr-12 ${formData.confirmPassword ? (passwordsMatch ? "success" : "error") : ""}`}
          ...
          {/* OAuth Buttons */}
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => handleOAuth("github")} className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl border border-white/10 bg-white/[0.03] hover:bg-white/[0.06] hover:border-white/20 transition-all text-sm font-semibold text-text-primary">
            ...
            <button onClick={() => handleOAuth("google")} className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl border border-white/10 bg-white/[0.03] hover:bg-white/[0.06] hover:border-white/20 transition-all text-sm font-semibold text-text-primary">
          ...
          <p className="text-center text-sm text-text-muted mt-8">
            ...
            <Link href="/login" className="text-accent-cyan font-semibold hover:text-accent-cyan/80 transition-colors">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
